...
void myGLWidget::settingUpMeshBasic( double isDefaultPos, Vector3d camPos )
{
    ...
    message = message + "F2F construction : "
				+ DoubleToString(timer.GetTotal()/1000.0)+ "\n";
...
void myGLWidget::renderToGeodesicRepresentation()
{
    ...
    timer.Stop();
    cout<<"Distance label construction :"
   ...
   timer.Stop();
   cout<<"Constructing colors from distance :"
		<< timer.GetTotal()/1000.0<<" s"<<endl;
...
void myGLWidget::renderToLaplaceRepresentation( int weightOption, int eigenCol )
{
    ...
    timer.Stop();
    cout<<"Calculating eigen decomposition: "
			<< timer.GetTotal()/1000.0<<" s"<<endl;
